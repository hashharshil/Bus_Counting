...
#Source of video
cap = cv2.VideoCapture(0)  #Test Files/example_03.mp4
...
while(cap.isOpened()):
   # Read an image from the video source
    ...
    #cnt_bus to count person inside the store
    cnt_bus = cnt_up - cnt_down
    if cnt_bus < 0:
        str_bus = "Bus is Empty"
    else:
        str_bus = 'In_Bus: ' + str(cnt_bus)
    ...
    cv2.putText(frame, str_bus ,(10,460),font,0.5,(0,255,0),2,cv2.LINE_AA)
    if cnt_bus>20:
